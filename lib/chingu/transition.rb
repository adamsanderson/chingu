# Todo: this was pulled from code used in another project, integrate it better with Chingu
class Transition
  ...
  def before
    ...
    # This would be a good place to register the timer with the gamestate
  end
  
  def after
    @options[:after].call if @options[:after]
    # This would be a good place to remove the transition
  end
  ...
  def compute_change(start_value, end_value)
    end_value.to_f - start_value.to_f
  end
  ...
  TWEEN_FUNCTIONS={
    ...
    :ease_out_quart =>lambda{|t, b, c, d| -c * ((t/d-1) ** 4 - 1) + b},  #return -c * (Math.pow (t/d-1, 4) - 1) + b
